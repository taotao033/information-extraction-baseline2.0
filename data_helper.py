...
class DataReader(object):
    ...
    def statistical_sub_obi_ner_labels_category_nummbers(self, train_ner_path, dev_ner_path):
        ...
        with open(train_ner_path, 'r') as train_f:
            for i, li in enumerate(tqdm(train_f.readlines())):
                li_ = li.split(" ")
                if li_[2].replace("\n", '') == '':
                    continue
                train_labels_category.add(str(li_[2]).replace("\n", ''))
                train_labels_all.append(str(li_[2]).replace("\n", ''))
    ...
    def _cal_mark_slot(self, spo_list, sentence, token_idx_list):
        ...
        for sub in sub_temp.difference(sub_obj_intersection_line):
            ...
            for s_idx in s_idx_list:
                if len(s_idx) == 1:
                    mark_list[s_idx[0]] = 'B-entity'
                else:
                    mark_list[s_idx[0]] = 'B-entity'
                    mark_list[s_idx[-1]] = 'I-entity'
                    for idx in range(1, len(s_idx) - 1):
                        mark_list[s_idx[idx]] = 'I-entity'
        ...
        for obj in obj_temp.difference(sub_obj_intersection_line):
            ...
            for o_idx in o_idx_list:
                if len(o_idx) == 1:
                    mark_list[o_idx[0]] = 'B-entity'
                else:
                    mark_list[o_idx[0]] = 'B-entity'
                    mark_list[o_idx[-1]] = 'I-entity'
                    for idx in range(1, len(o_idx) - 1):
                        mark_list[o_idx[idx]] = 'I-entity'
        ...
        for sub_obj in sub_obj_intersection_line:
            ...
            for s_o_idx in s_o_idx_list:
                if len(s_o_idx) == 1:
                    mark_list[s_o_idx[0]] = 'B-entity'
                else:
                    mark_list[s_o_idx[0]] = 'B-entity'
                    mark_list[s_o_idx[-1]] = 'I-entity'
                    for idx in range(1, len(s_o_idx) - 1):
                        mark_list[s_o_idx[idx]] = 'I-entity'
    ...
    def get_sub_obj_entity_bio(self, output_path):
        ...
        # Add postag. 20190424
        # get entity bio format. 20190502
        ...
        with open(self.train_path, 'r', encoding='utf-8') as f:
            for line in tqdm(f.readlines()):
                ...
                word_list = [str(item["word"]).replace(" ", '') for item in dic["postag"] if item["word"] != ' ']
                sentence = ''
                for w in word_list:
                    sentence += w
    ...
    def get_confirm_rel_dict(self, path="./results_output/bilstm_output/test_predication.csv"):
        df = pd.read_csv(path, sep='\t')
        row_id_in_raw_data = df["row_id_in_raw_data"].tolist()
        subject = df["subject"].tolist()
        object = df["object"].tolist()
        rel_0_or_1 = df["label_prediction"].tolist()
        confirm_rel_dict = {}

        for i, rel in enumerate(rel_0_or_1):
            if rel == 0:
                continue
            if row_id_in_raw_data[i] not in confirm_rel_dict:
                temp = [(subject[i], object[i])]
                confirm_rel_dict[row_id_in_raw_data[i]] = temp
            else:
                temp_in = list(confirm_rel_dict[row_id_in_raw_data[i]])
                temp_in.append((subject[i], object[i]))
                confirm_rel_dict[row_id_in_raw_data[i]] = temp_in

        return confirm_rel_dict
    ...
    def convert_data_format_for_entity_aware_relation_classification_2(self,
        ...
        with open(output_train_path, "w") as output_train:
            with open(self.train_path, "r") as train_f:
                for row_id_train, li in enumerate(tqdm(train_f.readlines()[2:]), 1):
                    # verify that the input format of each line meets the format
                    if not self._is_valid_input_data(li):
                        print('Format is error')
                        return None
                    ...
                    sub_obj_dic_temp = {}
                    for spo in dic["spo_list"]:
                        sub = str(spo['subject']).replace(" ", '').replace("《", '').replace("》", '').lower()
                        obj = str(spo['object']).replace(" ", '').replace("《", '').replace("》", '').lower()
                        rel = self._p_map_eng_dict.get(
                            str(spo["predicate"]).replace(" ", '').replace("《", '').replace("》", '').lower())
                        sub_obj_keys = sub + "\t" + obj
                        if sub_obj_keys not in sub_obj_dic_temp.keys():
                            sub_obj_dic_temp[sub_obj_keys] = rel
                        else:
                            rel_in = sub_obj_dic_temp[sub_obj_keys]
                            if rel_in != rel:
                                sub_obj_dic_temp[sub_obj_keys] = rel_in + "\t" + rel
                    ...
                    sentence_term_list = [str(item['word']).replace(" ", '').lower() for item in dic['postag']
                                          if item["word"] != " "]
                    token_idx_list = self._get_token_idx(sentence_term_list, sentence)
                    for k, v in sub_obj_dic_temp.items():
                        sub_, obj_ = k.split("\t")
                        multi_labels_relation = v
                        s_idx_list = self._cal_item_pos(self._add_item_offset(sub_, sentence),\
                                                        token_idx_list)            ##[[0,1,2]]
                        if len(s_idx_list) == 0:
                            continue
                        try:
                            sentence_term_list_copy = copy.deepcopy(sentence_term_list)
                            sentence_term_list_copy.insert(s_idx_list[0][0], "<s>")
                            sentence_term_list_copy.insert(s_idx_list[0][-1] + 2, "</s>")
                            sent_line = ''
                            for w in sentence_term_list_copy:
                                sent_line += w
                            sent_line = sent_line.strip()
                            ...
                            token_idx_list_ = self._get_token_idx(sentence_term_list_copy, sent_line)
                            o_idx_list = self._cal_item_pos(self._add_item_offset(obj_, sent_line), \
                                                            token_idx_list_)
                            if len(o_idx_list) == 0:
                                continue
                            sentence_term_list_copy.insert(o_idx_list[0][0], "<o>")
                            sentence_term_list_copy.insert(o_idx_list[0][-1] + 2, "</o>")
                            ...
                            output_train.write(sub_ + '\t' + obj_ + '\n' + multi_labels_relation + "\n\n")
                        except:    # Optimize the participle according to the subject and object
                            count_error_train += 1
                            sent_line_updated, words_seg_updated = \
                                self.deal_with_participle_less_cutting_problem(sentence, sub_, obj_)

                            token_idx_list_update = self._get_token_idx(words_seg_updated, sent_line_updated)
                            s_idx_list_u = self._cal_item_pos(self._add_item_offset(sub_, sent_line_updated), \
                                                               token_idx_list_update)
                            if len(s_idx_list_u) == 0:
                                continue
                            try:
                                words_seg_updated_copy = copy.deepcopy(words_seg_updated)
                                words_seg_updated_copy.insert(s_idx_list_u[0][0], "<s>")
                                words_seg_updated_copy.insert(s_idx_list_u[0][-1] + 2, "</s>")
                                sent_line = ''
                                for w in words_seg_updated_copy:
                                    sent_line += w
                                sent_line = sent_line.strip()

                                token_idx_list_update2 = self._get_token_idx(words_seg_updated_copy, sent_line)
                                o_idx_list_u = self._cal_item_pos(self._add_item_offset(obj_, sent_line), \
                                                                  token_idx_list_update2)
                                if len(o_idx_list_u) == 0:
                                    continue
                                words_seg_updated_copy.insert(o_idx_list_u[0][0], "<o>")
                                words_seg_updated_copy.insert(o_idx_list_u[0][-1] + 2, "</o>")
        ...
        with open(output_dev_path, "w") as output_dev:
            with open(self.dev_path, "r") as dev_f:
                for row_id_dev, li in enumerate(tqdm(dev_f.readlines()), 1):
                    # verify that the input format of each line meets the format
                    if not self._is_valid_input_data(li):
                        print('Format is error')
                        return None
        ...
        confirm_rel_dict = self.get_confirm_rel_dict(path="./results_output/bilstm_output/test_predication.csv")
        with open(output_test_path, "w") as output_test:
            with open(self.test_path, "r") as test_f:
                for row_id_test, li in enumerate(tqdm(test_f.readlines()), 1):
                    # verify that the input format of each line meets the format
                    if not self._is_valid_input_data(li):
                        print('Format is error')
                        return None
                    ...
                    for spo in dic["spo_list"]:
                        sub = str(spo['subject']).replace(" ", '').replace("《", '').replace("》", '').lower()
                        obj = str(spo['object']).replace(" ", '').replace("《", '').replace("》", '').lower()
                        if row_id_test not in confirm_rel_dict.keys() or \
                                (sub, obj) not in confirm_rel_dict[row_id_test]:
                            continue
                        relation = 'ACT'
                        s_idx_list = self._cal_item_pos(self._add_item_offset(sub, sentence), \
                                                        token_idx_list)  ##[[0,1,2]]
                        ...
                        try:
                            ...
                            s_idx_list_ = self._cal_item_pos(self._add_item_offset(sub, sent_line_updated), \
                                                             token_idx_list_)
                            if len(s_idx_list_) == 0:
                                continue
                            try:
                                words_seg_updated_copy = copy.deepcopy(words_seg_updated)
                                words_seg_updated_copy.insert(s_idx_list_[0][0], "<s>")
                                words_seg_updated_copy.insert(s_idx_list_[0][-1] + 2, "</s>")
                                sent_line = ''
                                for w in words_seg_updated_copy:
                                    sent_line += w
                                sent_line = sent_line.strip()
                                ...
                                o_idx_list_ = self._cal_item_pos(self._add_item_offset(obj, sent_line), \
                                                                 token_idx_list_)
                                if len(o_idx_list_) == 0:
                                    continue
                                words_seg_updated_copy.insert(o_idx_list_[0][0], "<o>")
                                words_seg_updated_copy.insert(o_idx_list_[0][-1] + 2, "</o>")
        ...
        with open(output_bad_case_path, "a+") as bad_case:
            ...
            log_f_dev = "Wrong number of sentences in dev set: " + str(count_error_dev) + "\n" + \
                        ...
                        "Number of unresolved sentences in dev set: " + str(count_error_dev -
                                                                            count_error_handled_dev) + '\n'
        ...
        bad_case.close()
            ...
            ####The following is a sample output:
            # 0	如何 演 好 自己 的 角色 ， 请 读 《 演员自我修养 》 《 <s> 喜剧之王 </s> 》 <o> 周星驰 </o> 崛起 于 穷困潦倒 之中 的 独门 秘笈
            # 喜剧之王  周星驰
            # ACT

            # 1	<s> 茶树茶网蝽 </s> ， Stephanitis chinensis Drake ， 属 <o> 半翅目 </o> 网蝽科冠网椿属 的 一种 昆虫
            # 茶树茶网蝽 半翅目
            # BO

            # 21      《 <s> 狙击部队 </s> 》 是 由 江苏海润影视制作有限公司 出品 的 电视剧 ， 由 <o> 赵浚凯 </o> 执导 ， 王珂 、 赵聪 、 刘鑫 等 主演
            # 狙击部队        赵浚凯
            # WC      DIR


    # The method do not add subject and object markers, and add "other" relationship,
    # later comparative experimental results. The experiment turned out badly. Therefore, this method is not used
    # to process data
                ...
                for li in tqdm(train_f.readlines()):
                    ...
                    for s in entiies:
                        for o in entiies:
                            ...
                            temp.append(o)
                            temp_ = tuple(temp)
                            if temp_ not in sub_obj_tuple_list:
    ...
    def analyse_spo_list(self):
        train_count = 0
        train_out = open("train_same_sub_obi_dif_rel_samples.json", 'w', encoding="utf-8")
        with open(self.train_path, "r") as train_f:
            for row_id, li in enumerate(tqdm(train_f.readlines())):
                # verify that the input format of each line meets the format
                if not self._is_valid_input_data(li):
                    print('Format is error')
                    return None
                ...
                for spo in dic["spo_list"]:
                    ...
                    sub_obj = sub + obj
                    p = [spo["predicate"]]
                    if sub_obj not in sub_obj_p_dic.keys():
                        sub_obj_p_dic[sub_obj] = p
                    else:
                        p_in = sub_obj_p_dic[sub_obj]
                        if p != p_in:
                            train_count += 1
                            train_out.write(json.dumps(dic, ensure_ascii=False))
                            train_out.write('\n')
                            break
                            #print("In training set, multi lables relationship line: " + str(row_id))
        train_out.close()
        train_f.close()

        dev_count = 0
        dev_out = open("dev_same_sub_obi_dif_rel_samples.json", 'w', encoding="utf-8")
        with open(self.dev_path, "r") as dev_f:
            for row_id, li in enumerate(tqdm(dev_f.readlines())):
                # verify that the input format of each line meets the format
                if not self._is_valid_input_data(li):
                    print('Format is error')
                    return None
                ...
                for spo in dic["spo_list"]:
                    ...
                    if sub_obj not in sub_obj_p_dic.keys():
                        ...
                        if p != p_in:
                            dev_count += 1
                            dev_out.write(json.dumps(dic, ensure_ascii=False))
                            dev_out.write('\n')
                            break
                            #print("In dev set, multi lables relationship line: " + str(row_id))
        dev_out.close()
        dev_f.close()

        print("In training set, the total number of multi lables relationship is : " + str(train_count) + "\n")
        print("In dev set, the total number of multi lables relationship is : " + str(dev_count))
...
if __name__ == "__main__":
    ...
    # data_reader.get_confirm_rel_dict(path="./results_output/bilstm_output/test_predication.csv")
    # data_reader.convert_data_format_for_entity_aware_relation_classification_2(
    #     p_eng_dict_path="./data/target_labels/relationship_zh2en",
    #     output_train_path="./relationship_extraction/"
    #                       "entity-aware-relation-classification_49_relationship/"
    #                       "data/train",
    #     output_dev_path="./relationship_extraction/"
    #                       "entity-aware-relation-classification_49_relationship/"
    #                       "data/dev",
    #     output_test_path="./relationship_extraction/"
    #                       "entity-aware-relation-classification_49_relationship/"
    #                       "data/test",
    #     output_bad_case_path="./relationship_extraction/"
    #                       "entity-aware-relation-classification_49_relationship/data/bad_case_details.txt")

    # data_reader.analyse_spo_list()
    ...
    # data_ reader.get_sub_obj_entity_bio(output_path="./sub_obj_extraction/bert-chinese-ner/data/") ##Convert the training set and the dev set to the bio format.
    # data_reader.statistical_sub_obi_ner_labels_category_nummbers(train_ner_path=
    #                                                              "./sub_obj_extraction/bert-chinese-ner/data/train.txt",
    #                                                              dev_ner_path=
    #                                                              "./sub_obj_extraction/bert-chinese-ner/data/dev.txt")
    ...
    data_reader.add_relationship_other_in_raw_data(train_output_path=
                                                   ...
                                                   "data_raw/20190509_rel_50/train.json",
                                                   dev_output_path=
                                                   "./relationship_extraction/"
                                                   "entity-aware-relation-classification_49_multilabels_rel/"
                                                   "data_raw/20190509_rel_50/dev.json")

    #add "other" relationship in the raw data and analyse it.
    data_reader2 = DataReader(train_path="./relationship_extraction/"
                                         "entity-aware-relation-classification_49_multilabels_rel/"
                                         "data_raw/20190509_rel_50/train.json",
    ...
    data_reader2.convert_data_format_for_entity_aware_relation_classification_2(
        ...
        output_train_path="./relationship_extraction/"
                          "entity-aware-relation-classification_49_multilabels_rel/"
                          "data/20190509_rel_50/train",
        output_dev_path="./relationship_extraction/"
                          "entity-aware-relation-classification_49_multilabels_rel/"
                          "data/20190509_rel_50/dev",
        output_test_path="./relationship_extraction/"
                          "entity-aware-relation-classification_49_multilabels_rel/"
                          "data/20190509_rel_50/test",
        output_bad_case_path="./relationship_extraction/"
                          "entity-aware-relation-classification_49_multilabels_rel/data/20190509_rel_50/bad_case_details.txt")