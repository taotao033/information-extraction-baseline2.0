...
class EntityAttentionLSTM:
    def __init__(self, sequence_length, num_classes,
        ...
        with tf.variable_scope('output'):
            ...
            self.predictions = tf.cast(tf.greater_equal(tf.sigmoid(self.logits), 0.45), tf.int32)
            # self.predictions = tf.argmax(self.logits, 1, name="predictions")

        # Calculate mean cross-entropy loss
        with tf.variable_scope("loss"):
            # losses = tf.nn.softmax_cross_entropy_with_logits_v2(logits=self.logits, labels=self.input_y)
            losses = tf.losses.sigmoid_cross_entropy(logits=self.logits, multi_class_labels=self.input_y)
        ...
        with tf.variable_scope("accuracy"):
            ...
            correct_attrs_op = tf.equal(tf.cast(tf.greater_equal(tf.sigmoid(self.logits), 0.45), tf.int32),
                                        tf.cast(self.input_y, tf.int32))
            self.accuracy = tf.reduce_mean(tf.reduce_min(tf.cast(correct_attrs_op, tf.float32), 1), name="accuracy")
