...
def load_data_and_labels(path, features_saved_path, type):
    if not os.path.exists(features_saved_path + type + "_features.csv"):
        ...
        for idx in tqdm(range(0, len(lines), 4)):
            ...
            sub_name = lines[idx + 1].split('\t')[0]
            obj_name = lines[idx + 1].split('\t')[1]
            ...
            try:
                ...
                print(sentence)
            if type == 'test':
                data.append([id, sentence, subject, object, sub_name, obj_name, relation])
            else:
        ...
        if type == 'test':
            df = pd.DataFrame(data=data, columns=["id", "sentence", "subject", "object",
                                                  "sub_name", "obj_name", "relation"])
        else:
    ...
    # Label Data
    if type == 'test':
        y = [[0]] * len(df['relation'].tolist())
    elif type == 'dev':
        y = []
        dev_gold = []
        for la in df['relation'].tolist():
            if la == 'OTHER':
                y.append([0])
                dev_gold.append(0)
            else:
                y.append([1])
                dev_gold.append(1)
        dev_gold_out = open("./runs_logs/dev_gold.txt", 'w')
        for g in dev_gold:
            dev_gold_out.write(str(g) + '\n')
        dev_gold_out.close()
    elif type == 'train':
        y = []
        for la in df['relation'].tolist():
    ...
    return sentence, labels, subject, object, pos1, pos2
