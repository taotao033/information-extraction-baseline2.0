...
class Logger:
    ...
    def logging_eval(self, step, loss, accuracy, truly, predictions):
        ...
        precision = precision_score(truly, predictions, average='binary')
        recall = recall_score(truly, predictions, average='binary')
        f1 = f1_score(truly, predictions, average='binary')

        self.best_f1 = max(self.best_f1, f1)
        f1_log = "<<< (2)-BINARY CLASSIFICATION RELATIONSHIP EVALUATION RESULT ON DEV SET -- LIC2019 Information Extraction Task >>>:\n" \
